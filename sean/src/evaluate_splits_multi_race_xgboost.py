...
def prepare_data_for_training(data: pd.DataFrame, model_type: str = 'logistic_regression') -> pd.DataFrame:
    """
...
def train_xgboost_model(train_data: pd.DataFrame) -> dict:
    """
    Train an XGBoost model for safety car prediction.
    
    Args:
        train_data: Prepared training data
        
    Returns:
        Dictionary with trained model and preprocessing objects
    """
    try:
    ...
    exclude_cols = ['sc_in_prediction_window']
    feature_cols = [col for col in train_data.columns if col not in exclude_cols]
    
    X = train_data[feature_cols]
    y = train_data['sc_in_prediction_window'].astype(int)
    
    print(f"   XGBoost training data: {X.shape[0]} samples, {X.shape[1]} features")
    ...
    cv_scores = cross_val_score(xgb_model, X, y, cv=3, scoring='f1')
...
def predict_xgboost(model_dict: dict, test_data: pd.DataFrame) -> tuple:
    """
    Make predictions using XGBoost model.
    
    Args:
        model_dict: Dictionary with XGBoost model and metadata
    ...
    if len(available_features) < len(feature_names):
        missing_features = [col for col in feature_names if col not in available_features]
...
def get_xgboost_feature_importance(model_dict: dict) -> pd.DataFrame:
    """
    Get feature importance from XGBoost model.
    
    Args:
        model_dict: Dictionary with XGBoost model
...
def demo_xgboost_approach():
    """
    Demo the balanced approach with XGBoost model.
    """
    print("🚀 BALANCED SEASON COLLECTION WITH XGBOOST")
    print("="*60)
    ...
    if len(train_data) > 0 and len(test_data) > 0:
        ...
        train_prepared = prepare_data_for_training(train_data, 'xgboost')
        test_prepared = prepare_data_for_training(test_data, 'xgboost')
        
        print(f"\n🧪 TESTING XGBOOST TRAINING...")
        print("-"*50)
        
        # Train XGBoost model
        xgb_model_dict = train_xgboost_model(train_prepared)
        
        if xgb_model_dict is None:
        ...
        # Make predictions
        xgb_predictions, xgb_probabilities = predict_xgboost(xgb_model_dict, test_prepared)
        
        # Evaluate XGBoost
        target_col = 'sc_in_prediction_window'
        y_true = test_prepared[target_col].values
        
        print(f"\n📊 XGBOOST EVALUATION RESULTS")
        print("="*50)
        detailed_evaluation(y_true, xgb_predictions)
        
        # Feature importance
        print(f"\n🔍 XGBOOST FEATURE IMPORTANCE (TOP 15)")
        print("-"*50)
        xgb_importance = get_xgboost_feature_importance(xgb_model_dict)
        ...
        try:
            ...
            lr_trainer = TemporalModelTrainer()
            lr_model = lr_trainer.train([train_prepared])
            
            # Make LR predictions on original test data for compatibility
            ...
            lr_predictions_obj = lr_model.predict(test_data_for_prediction, [])
            lr_predictions = [p.prediction for p in lr_predictions_obj]
            
            print(f"\n📊 LOGISTIC REGRESSION EVALUATION")
            print("-"*40)
            detailed_evaluation(y_true, lr_predictions)
            ...
            comparison_data = {
                ...
                'Accuracy': [
                    accuracy_score(y_true, xgb_predictions),
                    accuracy_score(y_true, lr_predictions)
                ],
                'Precision': [
                    precision_score(y_true, xgb_predictions, zero_division=0),
                    precision_score(y_true, lr_predictions, zero_division=0)
                ],
                'Recall': [
                    recall_score(y_true, xgb_predictions, zero_division=0),
                    recall_score(y_true, lr_predictions, zero_division=0)
                ],
                'F1': [
                    f1_score(y_true, xgb_predictions, zero_division=0),
                    f1_score(y_true, lr_predictions, zero_division=0)
                ]
            }
            ...
            print(f"\n🥇 Winner: {best_model} (F1: {best_f1:.3f})")
            
            return season_data, train_data, test_data, xgb_model_dict, lr_model
            
        except Exception as e:
            print(f"Logistic regression comparison failed: {e}")
            return season_data, train_data, test_data, xgb_model_dict, None
...
if __name__ == "__main__":
    # Use XGBoost comparison approach
    demo_xgboost_approach()