...
def main():
    ...
    prediction_horizon = int(args.prediction_horizon[0])
    ...
    # Optionally specify class weights to combat class imbalance
    cls_weight = {
        dataset['label_encoder'].class_to_idx["green"]: 1.0,
        dataset['label_encoder'].class_to_idx["red"]: 1.0,
        dataset['label_encoder'].class_to_idx["safety_car"]: 5.0,
        dataset['label_encoder'].class_to_idx["unknown"]: 1.0,
        dataset['label_encoder'].class_to_idx["vsc"]: 1.0,
        dataset['label_encoder'].class_to_idx["vsc_ending"]: 1.0,
        dataset['label_encoder'].class_to_idx["yellow"]: 1.0,
    }

    model_name = f"rocket_ridge_weighted_driver{"_".join(drivers)}"
    model = RocketClassifier(n_kernels=1000, class_weight=cls_weight)

    # model_name = f"rocket_rf_driver{"_".join(drivers)}_weighted"
    ...
    # Save model
    with open(evaluator.run_dir / "my_model.pkl", 'wb') as f:
