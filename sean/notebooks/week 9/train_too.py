...
def create_dataset_metadata_from_config(dataset_config, dataset, features_used="all"):
    ...
    return DatasetMetadata(
        ...
        } for s in sessions],  # Convert SessionConfig objects to dicts
...
def evaluate_on_different_test_set(model, test_data_path, evaluator, model_metadata, dataset_metadata):
...
def analyze_previous_results(results_dir="evaluation_results"):
    ...
    for json_file in sorted(json_files):
        ...
        if "metrics" in result:
            summary = {
                ...
                "target_f1": result["metrics"]["target_class_metrics"].get("f1", 0) if result["metrics"]["target_class_metrics"] else 0,
                "target_recall": result["metrics"]["target_class_metrics"].get("recall", 0) if result["metrics"]["target_class_metrics"] else 0,
                "target_precision": result["metrics"]["target_class_metrics"].get("precision", 0) if result["metrics"]["target_class_metrics"] else 0
            }
        else:
            summary = {
                "file": json_file.name,
    ...
    if "accuracy" in df.columns:
        ...
        print("="*100)
        print(f"{'Model':<20} {'Features':<25} {'Accuracy':<10} {'Target F1':<10} {'Recall':<10} {'Precision':<10}")
        print("-"*100)
        
        for _, row in df_success.iterrows():
            print(f"{row['model']:<20} {row['features']:<25} {row['accuracy']:<10.4f} "
...
results = evaluator.evaluate_model(
    ...
    dataset_metadata=dataset_metadata,
    model_metadata=create_model_metadata('logistic_regression', models['logistic_regression'], class_weight),
    class_names=class_names,
    target_class="safety_car"
)
...
high_weight_results = quick_eval("logistic_regression", custom_weights=high_safety_weights)
