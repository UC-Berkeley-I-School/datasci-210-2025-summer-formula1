...
from f1_etl import SessionConfig, DataConfig, create_safety_car_dataset, FixedVocabTrackStatusEncoder
import fastf1
...
def setup_experiment_config():
    ...
    CROSS_RACE_CONFIGS = {
        "silverstone_2024": {
            "session": SessionConfig(year=2024, race="British Grand Prix", session_type="R"),
            "description": "Silverstone 2024 Race"
        },
        "spa_2024": {
            "session": SessionConfig(year=2024, race="Belgian Grand Prix", session_type="R"),
            "description": "Spa 2024 Race"
        },
        "monza_2024": {
            "session": SessionConfig(year=2024, race="Italian Grand Prix", session_type="R"),
...
def create_dataset(scope_name, window_config, cache_dir="./f1_cache", training_scopes=None):
...
def evaluate_models_cross_race(fitted_models, X_cross_race, y_cross_race, cross_race_name):
    ...
    for model_name, model in fitted_models.items():
        ...
        try:
            ...
            results[model_name] = {
                ...
                "f1_macro": f1_score(y_cross_race, y_pred, average='macro', zero_division=0),
                "f1_weighted": f1_score(y_cross_race, y_pred, average='weighted', zero_division=0),
                "predictions": y_pred,
...
def compare_train_vs_cross_race(train_results, cross_race_results, comparison_title=""):
    ...
    print(f"\n{'='*80}")
    print(f"TRAIN vs CROSS-RACE COMPARISON{' - ' + comparison_title if comparison_title else ''}")
    print(f"{'='*80}")
    
    comparison_data = []
    
    for model_name in train_results.keys():
        if model_name in cross_race_results and "error" not in train_results[model_name] and "error" not in cross_race_results[model_name]:
            ...
            comparison_data.append({
                ...
                "Accuracy_Drop": train_result["accuracy"] - cross_result["accuracy"],
    ...
    for _, row in df.iterrows():
        ...
        generalization = "Good" if f1_drop < 0.1 else "Poor" if f1_drop > 0.3 else "Moderate"
        print(f"  {model:20s}: F1 drop = {f1_drop:+.3f}, Generalization = {generalization}")
    
    return df

def detailed_cross_race_analysis(cross_race_results, class_names, target_class="safety_car"):
...
def run_cross_race_experiment(train_scope, cross_race_key, window_config, cache_dir="./f1_cache"):
    """Run complete cross-race experiment: train on one race, test on another"""
    
    print(f"\n{'='*80}")
    print(f"CROSS-RACE EXPERIMENT")
    ...
    try:
        ...
        cross_race_dataset = create_cross_race_dataset(cross_race_key, window_config, cache_dir)
        X_cross_race, y_cross_race = prepare_cross_race_data(cross_race_dataset)
        
        # Step 5: Analyze cross-race data distribution
        cross_race_class_names, cross_race_dist = analyze_cross_race_distribution(cross_race_dataset, y_cross_race)
        ...
        cross_race_results = evaluate_models_cross_race(
            fitted_models, X_cross_race, y_cross_race, 
        ...
        comparison_df = compare_train_vs_cross_race(train_results, cross_race_results, comparison_title)
    ...
    for train_scope in TRAINING_SCOPES.keys():
        for cross_race_key in CROSS_RACE_CONFIGS.keys():
            for window_config in WINDOW_CONFIGS:
                ...
                result = run_cross_race_experiment(train_scope, cross_race_key, window_config)
                if result:
...
def create_cross_race_summary(all_results):
    ...
    for experiment in all_results:
        ...
        for model_name, train_result in experiment["train_results"].items():
            if "error" not in train_result and model_name in experiment["cross_race_results"]:
                ...
                if "error" not in cross_result:
                    summary_data.append({
                        ...
                        "Cross_F1": cross_result["f1_macro"],
                        "F1_Drop": train_result["f1_macro"] - cross_result["f1_macro"],
                        "Generalization": train_result["f1_macro"] - cross_result["f1_macro"]
                    })
